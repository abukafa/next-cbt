...
export default function SoalForm({ initialData = null, isEdit = false }) {
    ...
    <div className="mb-6">
      ...
      <div className="bg-white">
        <ReactQuill 
          ...
          className="h-32 mb-10" // Extra bottom margin because toolbar/editor height can overlap
        />
      </div>
    </div>
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
      ...
      {/* Question Editor */}
      <div>
        <h3 className="text-lg font-bold text-gray-900 mb-4">Pertanyaan Utama</h3>
        {renderEditor("Teks Pertanyaan", "soal")}
      </div>

      {/* Options Editor */}
      <div className="border-t border-gray-200 pt-6">
        <h3 className="text-lg font-bold text-gray-900 mb-4">Pilihan Jawaban</h3>
      ...
      <div className="border-t border-gray-200 pt-6 flex items-center justify-between">
        ...
        <div className="flex gap-4">
          <Button type="button" variant="secondary" onClick={() => router.push("/dashboard/bank-soal")}>
            Batal
          </Button>
