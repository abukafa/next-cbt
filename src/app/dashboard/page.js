...
export default function DashboardPage() {
  ...
  const uniqueClasses = ["Semua", ...new Set(rawJadwal.map(j => j.kelas).filter(Boolean))].sort();
  
  const filteredJadwal = filterKelas === "Semua" ? rawJadwal : rawJadwal.filter(j => j.kelas === filterKelas);
    ...
    <DashboardLayout>
      <div className="mb-6 flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 mt-1">Selamat Datang, {session?.user?.name || "User"}</p>
        </div>
      </div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 md:col-span-2">
              <h3 className="text-lg font-bold text-gray-900 mb-4">Distribusi Mapel per Guru</h3>
              <div className="h-72">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
              <h3 className="text-lg font-bold text-gray-900 mb-4">Siswa per Jurusan</h3>
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 mb-8">
            <h3 className="text-lg font-bold text-gray-900 mb-4">Distribusi Bank Soal per Mapel</h3>
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="mb-6">
        ...
        {statusUjian && (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                <h3 className="text-lg font-bold text-gray-900 mb-4">Penyelesaian Ujian</h3>
                <div className="h-64">
                  {statusUjian.chart_status_ujian?.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        ...
                        >
                          {statusUjian.chart_status_ujian.map((entry, index) => (
                            <Cell key={`cell-${index}`} fill={entry.name === 'Selesai' ? '#10b981' : '#f59e0b'} />
                          ))}
                        </Pie>
                    ...
                    <div className="h-full flex items-center justify-center text-gray-400">Belum ada data partisipasi ujian</div>
                  )}
                </div>
              </div>

              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 md:col-span-2">
                <h3 className="text-lg font-bold text-gray-900 mb-4">Peserta per Ujian Aktif</h3>
                <div className="h-64">
                  {statusUjian.chart_ujian_test?.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={statusUjian.chart_ujian_test} layout="vertical" margin={{ left: 50 }}>
                        ...
                        <Bar dataKey="Peserta" fill="#8b5cf6" radius={[0, 4, 4, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  ) : (
                    <div className="h-full flex items-center justify-center text-gray-400">Belum ada data partisipasi ujian</div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center flex-wrap gap-4">
                <h3 className="text-lg font-bold text-gray-900">Jadwal Ujian Terbaru</h3>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-emerald-50 border-b border-emerald-100 text-sm font-bold text-emerald-900">
                      <th className="px-4 py-3 text-center w-12 border-r border-emerald-100">No</th>
                      <th className="px-4 py-3 border-r border-emerald-100">Hari & Tanggal</th>
                      <th className="px-4 py-3 text-center border-r border-emerald-100">Jam Ke-</th>
                      <th className="px-4 py-3 text-center border-r border-emerald-100">Waktu</th>
                      <th className="px-4 py-3 border-r border-emerald-100">Mata Pelajaran</th>
                      {!isSiswa && <th className="px-4 py-3 text-center">Token</th>}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200">
                    {sortedDates.length > 0 ? (
                      sortedDates.map((dateStr, dayIndex) => {
                        ...
                        exams.sort((a, b) => new Date(a.tgl_mulai) - new Date(b.tgl_mulai));
                        
                        return exams.map((exam, examIndex) => {
                          const startTime = new Date(exam.tgl_mulai);
                          const endTime = new Date(startTime.getTime() + (exam.waktu || 0) * 60000);
                          ...
                          const dateFormatted = startTime.toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });

                          return (
                            <tr key={exam.id} className="hover:bg-gray-50 transition-colors">
                              {examIndex === 0 && (
                                <>
                                  <td className="px-4 py-3 text-center border-r border-gray-200 align-top bg-white" rowSpan={exams.length}>
                                    <span className="font-semibold text-gray-700">{dayIndex + 1}</span>
                                  </td>
                                  <td className="px-4 py-3 font-medium text-gray-900 border-r border-gray-200 align-top bg-white" rowSpan={exams.length}>
                                    {dateFormatted}
                                  </td>
                                </>
                              )}
                              ...
                              <td className="px-4 py-3 text-gray-800 border-r border-gray-200">
                                <div className="font-semibold">{exam.nama_mapel}</div>
                                <div className="text-xs text-gray-500 mt-0.5">{exam.nama_ujian} &bull; Kelas {exam.kelas} {exam.jurusan}</div>
                              </td>
                              {!isSiswa && (
                      ...
                      <tr>
                        <td colSpan={isSiswa ? 5 : 6} className="px-6 py-12 text-center text-gray-500 bg-gray-50/50">
                          <div className="flex flex-col items-center justify-center">
                            <BookOpen className="h-10 w-10 text-gray-300 mb-3" />
                            <p>Tidak ada jadwal ujian untuk kelas yang dipilih.</p>
                          </div>
                        </td>
                      </tr>
                    )}
