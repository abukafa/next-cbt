...
export default function HasilUjianPage() {
  ...
  const filteredExams = exams.filter(tes => 
    ...
    <DashboardLayout>
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600">Pantau progres dan hasil ujian siswa Anda.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900">Tidak ada data ujian</h3>
          <p className="text-gray-500">Anda belum membuat jadwal ujian apapun.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredExams.map((tes) => (
            <div key={tes.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow flex flex-col">
              <div className="p-6 flex-1">
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-lg font-bold text-gray-900 line-clamp-2" title={tes.nama_ujian}>
                    {tes.nama_ujian}
                  </h3>
                ...
                <div className="space-y-2 mb-6">
                  ...
                  <div className="flex items-start gap-3 text-sm text-gray-600">
                    ...
                    <div className="flex flex-col">
                      <span>{new Date(tes.tgl_mulai).toLocaleDateString("id-ID", {dateStyle: 'medium'})}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-auto">
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-gray-500 font-medium">Progress Peserta</span>
                  ...
                  <div className="w-full bg-gray-100 rounded-full h-2">
                    <div 
                      ...
                      style={{ width: tes.jml_peserta > 0 ? `${(tes.jml_selesai / tes.jml_peserta) * 100}%` : '0%' }}
                    ></div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 px-6 py-4 border-t border-gray-100">
                <button
                  onClick={() => router.push(`/dashboard/hasil-ujian/${tes.id}`)}
