...
import { ChevronLeft, BookOpen, Clock, Users, Trash2, CheckSquare, Search, RefreshCw, Printer } from "lucide-react";
    ...
    <DashboardLayout>
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4 print:hidden">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-900">Detail Hasil Ujian</h1>
        </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6 print:border-none print:p-0 print:mb-4">
            <h2 className="text-xl font-bold text-gray-900 mb-4">{data.nama_ujian}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 print:grid-cols-4 gap-6 print:gap-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-xs text-gray-500 font-medium uppercase">Mata Pelajaran</p>
                  <p className="font-semibold text-gray-900">{data.nama_mapel}</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="p-2 bg-purple-50 text-purple-600 rounded-lg"><Users size={20} /></div>
                <div>
                  <p className="text-xs text-gray-500 font-medium uppercase">Kelas Ujian</p>
                  <p className="font-semibold text-gray-900">{data.kelas}</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="p-2 bg-emerald-50 text-emerald-600 rounded-lg"><CheckSquare size={20} /></div>
                <div>
                  <p className="text-xs text-gray-500 font-medium uppercase">Jml Soal</p>
                  <p className="font-semibold text-gray-900">{data.jumlah_soal} Butir</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="p-2 bg-orange-50 text-orange-600 rounded-lg"><Clock size={20} /></div>
                <div>
                  <p className="text-xs text-gray-500 font-medium uppercase">Alokasi Waktu</p>
                  <p className="font-semibold text-gray-900">{data.waktu} Menit</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden print:border-none print:shadow-none">
            <div className="p-4 border-b border-gray-200 flex flex-col sm:flex-row justify-between gap-4 items-center bg-gray-50 print:hidden">
              <h3 className="font-bold text-gray-900">Daftar Peserta ({participants.length})</h3>
              ...
              <table className="w-full text-left border-collapse print:text-xs">
                <thead>
                  <tr className="bg-white border-b border-gray-200 text-sm print:text-xs">
                    <th className="px-6 py-4 print:px-2 print:py-2 font-semibold text-gray-600">No</th>
                    <th className="px-6 py-4 print:px-2 print:py-2 font-semibold text-gray-600">Peserta</th>
                    <th className="px-6 py-4 print:px-2 print:py-2 font-semibold text-gray-600">Status</th>
                    <th className="px-6 py-4 print:px-2 print:py-2 font-semibold text-gray-600">Waktu Mulai</th>
                    <th className="px-6 py-4 print:px-2 print:py-2 font-semibold text-gray-600 text-center">Jml Benar</th>
                    <th className="px-6 py-4 print:px-2 print:py-2 font-semibold text-gray-600 text-center">Nilai Akhir</th>
                    <th className="px-6 py-4 print:hidden font-semibold text-gray-600 text-right">Aksi</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {filteredParticipants.length === 0 ? (
                    ...
                    filteredParticipants.map((p, idx) => (
                      <tr key={p.id_ikut_ujian} className="hover:bg-gray-50 transition-colors border-b border-gray-50 print:border-gray-200">
                        <td className="px-6 py-4 print:px-2 print:py-2 text-sm print:text-xs text-gray-500">{idx + 1}</td>
                        <td className="px-6 py-4 print:px-2 print:py-2">
                          <div className="font-medium text-gray-900 print:whitespace-nowrap">{p.nama}</div>
                        ...
                        <td className="px-6 py-4 print:px-2 print:py-2">
                          {p.status === "Y" ? (
                            ...
                            <span className="inline-flex items-center gap-1.5 bg-amber-100 print:bg-transparent text-amber-700 px-2.5 py-1 rounded-full text-xs font-bold">
                              <span className="w-1.5 h-1.5 bg-amber-500 rounded-full animate-pulse print:hidden"></span> Mengerjakan
                            </span>
                          )}
                        </td>
                        ...
                        <td className="px-6 py-4 text-right print:hidden">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <button
                              onClick={() => handleAction('reset', p.id_ikut_ujian, p.nama)}
                              disabled={processingId === p.id_ikut_ujian}
